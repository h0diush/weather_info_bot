...
def response_to_command_info_city_from_db(city: City,
                                          info_the_city: dict) -> str:
...
def response_to_command_info_city_from_user(city: City,
                                            info_the_city: dict) -> str:
    text = (f"Вы выбрали город: {city} ({info_the_city['country']})\n"
            f"Температура составляет {info_the_city['temperature']} °C\n"
            f"Влажность составляет {info_the_city['humidity']} %\n"
            f"Погода, если описать двумя словами: "
            f"{info_the_city['description']}\n")
